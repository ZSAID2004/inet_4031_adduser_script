...
def main():
    # Ask the user if they want to do a dry run or not.
    dry_run_input = input("Dry run? (Y/N): ").strip().upper()
    ...
    for line in sys.stdin:
        ...
        if match or len(fields) != 5:
            if dry_run:
                if match:
                    print("[DRY-RUN] Skipping line because it starts with '#':", line.strip())
                elif len(fields) != 5:
                    print("[DRY-RUN] Skipping line because it doesn't have 5 fields (found %d): %s" % (len(fields), line.strip()))
            continue
        ...
        cmd = "/usr/sbin/adduser --disabled-password --gecos '%s' %s" % (gecos,username)  # The first time you run the code, you should leave the 'os.system(cmd' commented out using '#' so you can test the code safely.
    ...
    if dry_run:
        ...
        print("Normal run complete. All users created successfully.")
